//
//  RequestAPI_ConfirmDateOff.swift
//  Employee
//
...
extension AQUA_ConfirmDateOff {
    func getListConfirmDateOff() {
        if((self.tbvConfirmDateOff.arrDataTbv?.count)! > 0){
            self.tbvConfirmDateOff.arrDataTbv?.removeAllObjects()
        }
        self.showCircleLoadingInVC()
        let url = String(format: "%@%@", Constant.DOMAIN,API.GET_LIST_REGISTER_DATEOFF_FORMANAGER)
                                ...
                                if(arrAbsents.count > 0) {
                                    for absentJson in arrAbsents {
                                        if let absentsItem = Model_Absent.loadAbsent(absentJson) {
                                            self.tbvConfirmDateOff.arrDataTbv?.add(absentsItem)
                                        }
                                    }
                                    self.tbvConfirmDateOff.reloadData()
                                }
                            }
                        }else{
